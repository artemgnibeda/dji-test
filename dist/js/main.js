...
$(document).ready(function () {
  ...
  function checkNewstText() {
    var allNews = $('.js-news-text'),
      lengthRow = 70;
    allNews.each(function () {
      if (!$(this).siblings('js-show-more').length) {
        var str = $(this).text();
        if (str.length > lengthRow) {
          var firstSet = str.substring(0, lengthRow);
          var secdHalf = str.substring(lengthRow, str.length);
  ...
  $(window).scroll(function () {
    if ($(window).scrollTop() == $(document).height() - $(window).height()) {
      ...
      $.ajax({
        ...
        type: 'GET',
        timeout: 600000,
        success: function success(e, textStatus) {
